...
async function expectResult(fn, arg, {
  error,
  value,
  expectSync = false,
...
describe("gensync({})", () => {
  ...
  describe("'errback' async handler", async () => {
    test("success", async () => {
      const fn = gensync({
        sync: throwTestError,
        errback: (...args) => process.nextTick(() => args.pop()(null, JSON.stringify(args)))
      });
      await expectResult(fn, 42, {
        value: "[42]"
      });
    ...
    test("failure", async () => {
      const fn = gensync({
        sync: throwTestError,
        errback: (...args) => process.nextTick(() => args.pop()(JSON.stringify(args)))
      });
      await expectResult(fn, 42, {
        error: "[42]"
      });
